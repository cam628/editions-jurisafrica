...
function PenantContent() {
  ...
  // Render tab panels (to reuse for desktop and mobile)
  const renderTabPanel = (tabId: typeof tabs[number]['id']) => {
    if (tabId === 'dernier') {
      return (
      ...
      <section className="container-6xl py-10 relative z-10">
        ...
        <div className='no-bg'>
          <div className="tab-container">
            <div className="tab-header">
              {tabs.map(t => (
                <div key={t.id} className="tab-item">
                  ...
                  {/* Mobile inline panel under each title */}
                  <div
                    id={`panel-${t.id}`}
                    className={`tab-panel-mobile ${activeTab === t.id ? 'open' : ''}`}
                  >
                    {activeTab === t.id && (
            ...
            {/* Desktop content area below the tabs */}
